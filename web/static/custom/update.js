...
function check_reconpoint_update() {
  if (
    ...
    window.open("https://github.com/khulnasoft/reconpoint/releases", "_blank");
  } else {
    Swal.fire({
      title: "Checking reconPoint latest version...",
      ...
      .then(function (response) {
        ...
        if (response["description"] == "RateLimited") {
          ...
          Swal.fire({
            ...
            text: "You are running the latest version of reconPoint!",
            icon: "info",
          });
        }
      });
...
function update_available(latest_version_number, changelog) {
  ...
  Promise.all([
    ...
    Swal.fire({
      ...
      html: `
          ${changelogStyle}
          <h5>reconPoint's new update ${latest_version_number} is available, please follow the update instructions.</h5>
...
function showAfterUpdatePopup() {
  ...
  const currentVersion = document.body.getAttribute("data-reconpoint-version");
  ...
  if (lastShownVersion !== currentVersion) {
    ...
    // Swal.fire({
    //   title: isFirstRun ? "Welcome to reconPoint!" : "Thanks for updating!",
    ...
    // });
    Swal.fire({
      title: "Thanks for using reconPoint!",
      ...
      if (result.isConfirmed) {
        window.open(`https://reconpoint.wiki/whats-new/${currentVersion.replace(/\./g, "_")}`, "_blank");
      }
      localStorage.setItem("lastShownUpdateVersion", currentVersion);
    });
  }
