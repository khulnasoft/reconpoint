...
class TestScanEngineViews(BaseTestCase):
    ...
    def test_reconpoint_settings_view(self):
        """
        Tests the reconpoint settings view to ensure it returns the correct status code and template.
        """
        response = self.client.get(reverse("reconpoint_settings"))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "scanEngine/settings/reconpoint.html")
