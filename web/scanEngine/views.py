...
def duplicate_engine(request, id, slug=None):
    ...
    messages.add_message(
        request,
        messages.SUCCESS,
        f"Engine '{original_engine.engine_name}' successfully duplicated as '{new_name}'!",
    )
    return http.HttpResponseRedirect(_reverse_with_project(request, "scan_engine_index"))
...
def add_wordlist(request, slug=None):
    ...
    if len(files) == 1:
        if not _wordlist_is_txt_filename(files[0].name):
            ...
            return render(
                request,
                "scanEngine/wordlist/add.html",
                _wordlist_add_page_context(form),
            )
        ...
        if err:
            if err == "empty":
                ...
                form.add_error(
                    "short_name",
                    "Could not find a unique short name after many attempts.",
                )
            elif err == "encoding":
    ...
    for uploaded_file in files:
        ...
        try:
            ...
            messages.error(
                request,
                f"Skipped {uploaded_file.name}: invalid short name derived from filename.",
            )
            continue
        name = stem
...
def tool_specific_settings(request, slug=None):
    if request.method == "POST":
        ...
        if result.get("saved", 0) == 0 and result.get("errors", 0) > 0:
            ...
            return render(
                request,
                "scanEngine/settings/tool.html",
                _tool_settings_context(request),
            )
...
def duplicate_workflow(request, workflow_id, slug=None):
    ...
    messages.add_message(
        request,
        messages.SUCCESS,
        f"Workflow '{workflow.name}' duplicated as '{duplicated_name}'.",
    )
...
def duplicate_scan(request, scan_id, slug=None):
    ...
    messages.add_message(
        request,
        messages.SUCCESS,
        f"Scan '{scan.name}' duplicated as '{duplicated_name}'.",
    )
    ...
    if request.method == "POST":
        try:
            ...
            messages.add_message(
                request,
                messages.INFO,
                f"Workflow '{workflow_name}' successfully deleted!",
            )
        except PermissionError as e:
            response_data = {
                "status": False,
                "message": get_safe_user_message(e, logger),
            }
        except Exception:
            response_data = {
                "status": False,
                "message": "Oops! Workflow could not be deleted!",
            }
...
def delete_scan(request, scan_id, slug=None):
    ...
    if not scan.can_delete():
        response_data = {
            "status": False,
            "message": "Built-in scan configurations cannot be deleted!",
        }
        return http.JsonResponse(response_data)
    ...
    if request.method == "POST":
        try:
            ...
            messages.add_message(
                request,
                messages.INFO,
                f"Scan configuration '{scan_name}' successfully deleted!",
            )
        except PermissionError as e:
            ...
            response_data = {
                "status": False,
                "message": "Oops! Scan configuration could not be deleted!",
            }
            messages.add_message(
                request,
                messages.ERROR,
                "Oops! Scan configuration could not be deleted!",
            )
    else:
        response_data = {"status": False, "message": "Invalid request method"}
...
def duplicate_profile(request, profile_id, slug=None):
    ...
    messages.add_message(
        request,
        messages.SUCCESS,
        f"Profile '{profile.name}' duplicated as '{duplicated_name}'.",
    )
    ...
    if request.method == "POST":
        try:
            ...
            messages.add_message(
                request,
                messages.INFO,
                f"Profile '{profile_name}' successfully deleted!",
            )
        except PermissionError as e:
            response_data = {
                "status": False,
            ...
            response_data = {
                "status": False,
                "message": "Oops! Profile could not be deleted!",
            }
...
def duplicate_task(request, task_id, slug=None):
    ...
    messages.add_message(
        request,
        messages.SUCCESS,
        f"Task '{task.name}' duplicated as '{duplicated_name}'.",
    )
...
def duplicate_worker(request, worker_id, slug=None):
    ...
    messages.add_message(
        request,
        messages.SUCCESS,
        f"Worker '{worker.name}' duplicated as '{duplicated_name}'.",
    )
