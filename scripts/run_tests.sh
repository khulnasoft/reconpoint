...
show_help() {
    ...
    echo
    echo "Run tests for the reconPoint project in a VM environment."
    echo
...
# Reconpoint root directory inside the VM
RECONPOINT_ROOT='~/reconpoint'
...
(
    ...
    (cd "$REPO_ROOT" && tar -czf "$TEST_DIR/reconpoint-project.tar.gz" --exclude='docker/secrets' .)
...
EOF
    ...
    for i in {1..30}; do
        if ssh -p 2222 $SSH_OPTIONS -i ./id_ed25519 reconpoint@localhost echo "SSH is up" &>/dev/null; then
    ...
    ssh -p 2222 $SSH_OPTIONS -i ./id_ed25519 reconpoint@localhost << EOF
...
EOF
    ...
    scp -P 2222 $SSH_OPTIONS -i ./id_ed25519 "$TEST_DIR/reconpoint-project.tar.gz" reconpoint@localhost:~

    log "Decompressing project files on the VM..." $COLOR_CYAN
    ssh -p 2222 $SSH_OPTIONS -i ./id_ed25519 reconpoint@localhost << EOF
        sudo apt-get install git -y
        mkdir -p $RECONPOINT_ROOT
        tar -xzf ~/reconpoint-project.tar.gz -C $RECONPOINT_ROOT
        rm ~/reconpoint-project.tar.gz
        cd $RECONPOINT_ROOT
        cat > $RECONPOINT_ROOT/.git/config << EOG
[core]
...
[remote "origin"]
    url = https://github.com/KhulnaSoft/reconpoint.git
...
EOG
        cp $RECONPOINT_ROOT/.env-dist $RECONPOINT_ROOT/.env
EOF
    ...
    ssh -p 2222 $SSH_OPTIONS -i ./id_ed25519 reconpoint@localhost << EOF
        ...
        # Add reconpoint user to docker group
        sudo usermod -aG docker reconpoint
        newgrp docker
