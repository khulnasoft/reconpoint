"""
This module contains tests for the Makefile commands in the reconPoint project.
...
class TestMakefile(unittest.TestCase):
    """
    A test suite for verifying the functionality of the Makefile commands in the reconPoint project.
    ...
    expected_images = [
        f"ghcr.io/khulnasoft/reconpoint:reconpoint-web-v{RECONPOINT_VERSION}",
        f"ghcr.io/khulnasoft/reconpoint:reconpoint-postgres-v{RECONPOINT_VERSION}",
        f"ghcr.io/khulnasoft/reconpoint:reconpoint-redis-v{RECONPOINT_VERSION}",
        f"ghcr.io/khulnasoft/reconpoint:reconpoint-ollama-v{RECONPOINT_VERSION}",
        f"ghcr.io/khulnasoft/reconpoint:reconpoint-certs-v{RECONPOINT_VERSION}",
        f"ghcr.io/khulnasoft/reconpoint:reconpoint-proxy-v{RECONPOINT_VERSION}",
    ]
    ...
    def clean_secrets(self):
        ...
        """
        secrets_path = f"{RECONPOINT_PATH}/docker/secrets"
    ...
    def test_prune(self):
        ...
        # Check for reconPoint-related volumes
        volumes = self.client.volumes.list()
        reconpoint_volumes = [v for v in volumes if v.name.startswith("reconpoint_")]

        if reconpoint_volumes:
            volume_names = ", ".join([v.name for v in reconpoint_volumes])
            self.fail(f"reconPoint volumes still exist after pruning: {volume_names}")
