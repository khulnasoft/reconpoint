...
DB_USER=${POSTGRES_USER:-reconpoint}
...
perform_migration() {
    ...
    # Initialize new cluster with the reconpoint user as the superuser
...
until pg_isready -h localhost -p 5432 -U reconpoint; do
...
if [ -f "$PGDATA/pg_hba.conf" ]; then
    ...
    psql -h localhost -p 5432 -U reconpoint -c "SELECT pg_reload_conf();" || echo "Warning: Could not reload configuration"
else
