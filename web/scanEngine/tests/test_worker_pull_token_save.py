...
class TestSecatorWorkerPullTokenSaveLogic(BaseTestCase):
    ...
    def test_append_pull_token_update_field_returns_tuple_without_mutating_input(
        self,
    ) -> None:
    ...
    def test_prepare_pull_token_for_save_generates_when_pull_token_is_explicitly_updated(
        self,
    ) -> None:
    ...
    def test_prepare_pull_token_for_save_restores_existing_token_when_partial_without_pull_token(
        self,
    ) -> None:
        worker = self._create_worker()
