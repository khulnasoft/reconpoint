...
check_docker() {
  ...
  if ! [[ "$(printf '%s\n' "$min_version" "$version" | sort -V | head -n1)" = "$min_version" ]]; then
    log "Docker version $version is installed, but reconPoint requires version $min_version or higher." $COLOR_RED
...
check_docker_compose() {
  ...
  if ! [[ "$(printf '%s\n' "$min_version" "$version" | sort -V | head -n1)" = "$min_version" ]]; then
    log "Docker Compose version $version is installed, but reconPoint requires version $min_version or higher." $COLOR_RED
...
remove_old_images() {
  log "Checking for old reconPoint 2.0.7 Docker containers and images..." $COLOR_CYAN
  
  # Stop and remove all containers
  if docker ps -a --format '{{.Names}}' | grep -qE '^reconpoint-|^postgres$|^redis$'; then
    log "Stopping existing reconPoint containers..." $COLOR_YELLOW
    if ! make down; then
  ...
  declare -a old_images=(
    "docker.pkg.github.com/khulnasoft/reconpoint/reconpoint"
    "reconpoint-certs",
    "nginx",
    "postgres",
    "redis"
...
fix_volumes_permissions() {
  ...
  declare -a volumes=(
    "reconpoint_gf_patterns"
    "reconpoint_nuclei_templates"
    "reconpoint_scan_results"
    "reconpoint_wordlist"
  )
...
main() {
  ...
  log "Checking and installing reconPoint prerequisites..." $COLOR_CYAN
  ...
  if [ "$current_id" -ne 1000 ]; then
      ...
      INSTALL_TYPE="source"
      log "Build has been forced because your user ID is not the same as the pre-built images. If you want to use pre-built images, your current user installing reconPoint must be 1000." $COLOR_RED
  else
      if [ "$isNonInteractive" = false ]; then
  ...
  log "Installing reconPoint from $INSTALL_TYPE, please be patient as the installation could take a while..." $COLOR_CYAN
  sleep 5
  ...
  sleep 5
  make up && log "reconPoint is started!" $COLOR_GREEN || { log "reconPoint start failed!" $COLOR_RED; exit 1; }
  ...
  log "reconPoint is successfully installed and started!" $COLOR_GREEN
  log "\r\nThank you for installing reconPoint, happy recon!" $COLOR_GREEN

  # Get domain name from .env file
  domain_name=$(grep "^DOMAIN_NAME=" .env 2>/dev/null | cut -d'=' -f2)
  if [ -z "$domain_name" ]; then
    domain_name="reconpoint.example.com"
  fi
  
  log "\r\nreconPoint is available at: https://$domain_name/" $COLOR_GREEN
  log "\r\n⚠️  IMPORTANT CSRF Configuration Warning:" $COLOR_YELLOW
  log "Due to Django's CSRF protection, you MUST access reconPoint using the configured hostname." $COLOR_YELLOW
